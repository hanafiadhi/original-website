...
export async function openWithCookies(
  url: string,
  cookiesFile = path.resolve(process.cwd(), "cookies.json"),
  screenshotPath = path.resolve(process.cwd(), "foto", "screenshot.png")
) {
  ...
  const cookies: Cookie[] = JSON.parse(fs.readFileSync(cookiesFile, "utf-8"));
